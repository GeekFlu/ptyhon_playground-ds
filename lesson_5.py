...
def get_most_common_month(people):
    ...
    max_m = ''
    for person in people:
        months[person.birthday_month] = months.get(person.birthday_month) + 1
        if months.get(person.birthday_month) > max_:
            max_ = months.get(person.birthday_month)
            max_m = person.birthday_month
